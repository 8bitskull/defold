...
(defn make-vertex-buffer
  ...
  (let [byte-capacity (* vertex-capacity ^long (:size vertex-description))
...
(deftype VertexBufferShaderLink [request-id ^VertexBuffer vertex-buffer shader ^:unsynchronized-mutable expanded-attribute-locations]
  GlBind
...
(deftype VertexIndexBufferShaderLink [request-id ^VertexBuffer vertex-buffer ^IntBuffer index-buffer shader ^:unsynchronized-mutable expanded-attribute-locations]
  GlBind
  (bind [_this gl _render-args]
    (set! expanded-attribute-locations (bind-vertex-buffer-with-shader! gl request-id vertex-buffer shader))
...
(defn- update-vbo [^GL2 gl [vbo _] data]
  ...
  (let [^VertexBuffer vbuf (:vertex-buffer data)
    ...
    (gl/gl-buffer-data gl GL/GL_ARRAY_BUFFER (buffer-size-in-bytes buf) buf (usage-types (.usage vbuf)))
